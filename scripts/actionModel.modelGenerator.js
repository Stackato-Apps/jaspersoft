...
actionModel.showDynamicMenu = function(menuContext, event, className, coordinates, actionModelScriptTag, updateContextActionModel){
    ...
    actionModel.assembleMenuFromActionModel(menuContext, event, actionModel.menuListDom, actionModelScriptTag, updateContextActionModel);
...
actionModel.setMenuPosition = function(menuObj, event, coordinates){
    ...
    if(isNotNullORUndefined(coordinates)){
        if(isNotNullORUndefined(coordinates.menuLeft)){
            ...
            leftOffset = location.x + actionModel.MENUOFFSET;
        }
        if(isNotNullORUndefined(coordinates.menuTop)){
            topOffset = coordinates.menuTop;
        }else{
            topOffset = location.y + actionModel.MENUOFFSET;
        }
    }else{
 ...
 * @updateContextActionModel - optional method which can update context action model
 */
actionModel.assembleMenuFromActionModel = function(menuContext, event, contentParent, actionModelScriptTag, updateContextActionModel) {
    actionModel.data = null;
    actionModel.data = $(actionModelScriptTag).text;
    if(actionModelScriptTag === "navigationActionModel"){
        actionModel.data = primaryNavModule.JSON;
    }else{
        actionModel.data = ($(actionModelScriptTag).text.evalJSON()).evalJSON();
    }
    actionModel.noItemsYet = true;
    ...
    contextActionModel.each(function(thisAction){ actionModel.appendToMenu(thisAction, event, contentParent)});
...
actionModel.hideChildSubmenu = function(parent){
    var submenu = parent.childElements()[1];
    //applying style for fly-out submenu....
    ...
    /*
    if(isSupportsTouch()) {
        var positionInOpened = actionModel.openedSubMenus.indexOf(parent);
        if (positionInOpened > -1) {
            if((positionInOpened + 1) < actionModel.openedSubMenus.length) {
                var item = actionModel.openedSubMenus[positionInOpened + 1];
                item && actionModel.hideChildSubmenu(item);
            }
            actionModel.openedSubMenus = actionModel.openedSubMenus.without(parent);
        }
    }
    */
};
