...
AjaxRequester
   ...
   .addMethod('verifyAjaxResponse', function() {
    ...
    .addMethod('startResponseTimer', function() {
        this.responseTimer = setTimeout(function(){dialogs.popup.show($(ajax.LOADING_ID),true)},this.MAX_WAIT_TIME);
    })
...
function updateUsingResponseSubset(xmlhttp, fromLocation, toLoc) {
    ...
    if(typeof(jQuery) !== 'undefined'){
    	jQuery('script.fusioncharts').each(function() {
    ...
    if(typeof(jQuery) !== 'undefined' && true){
    	var scriptTags =  jQuery('script.jasperreports'),
    		sz  = scriptTags.size(),
    		idx = 0;
