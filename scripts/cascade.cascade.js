    ...
    if (undefined != allEnvelopes){
        for (var travEnv = 0; travEnv < allEnvelopes.length; travEnv++){
            ...
            if (env.controlType == 6 || env.controlType == 7) {
                ...
                var updatedList = env.optionsList;
...
function refreshControl(envelope){
    ...
    if ( envelope.controlType == 3 || envelope.controlType == 4 ||
        ...
        if (!envelope.permanent) {
            ...
            dwr.util.addOptions(controlName, envelope.optionsList, "value", "label");
        }
    ...
    modelEnv.optionsList = getSelections(envelope.controlName, envelope.controlType);
...
function updateRadiosAndCheckboxes(envelope, getParams, createElement, resetButtonClick, resetButtonText) {
    ...
    if (params) {
        if (params.parentElement) {
            ...
            envelope.optionsList.each(function(option) {
                var li = createElement(envelope, option, params);
                params.parentElement.insert(li);
            });

            //Reset Button
            if (envelope.optionsList.first() && !envelope.mandatory && (envelope.controlType ==  8 || envelope.controlType == 9)) {
...
*/
                ...
                envelope.optionsList = optionsMap.valSet();
//            }
        }
    } else {
...
function getControlsWithSelectedOptionsOnly(controlValues) {
    ...
    for (var i = 0; i < controlValues.length; i++) {
        ...
        if (ctl.optionsList) {
            for (var k = 0; k < ctl.optionsList.length; k++) {
                var opt = ctl.optionsList[k];
                if (opt.selected) {
                    var newOpt = {
                    ...
                    envelope.optionsList.push(newOpt);
                }
            }
        }
